...
exists $ENV{BLUESPECDIR} || die "Environment variable BLUESPECDIR must be set to the lib/ folder under Bluespec root";
exists $ENV{BLUELINKDIR} || die "Environment variable BLUELINKDIR must be set to the root of the BlueLink library";

use POSIX (strftime);
my $datestr = strftime("%H:%M:%S %B %d, %Y",localtime);
...
my @revisions = (
    {
        ...
        sdcfiles => ("250m_with_400ps_slack.sdc","mmMulticycle.sdc")
    }
);
...
# Virtual pin assignments for AFU (everything except ha_pclock)
...
for $rev (@revisions) {
    ...
    printf QSFH "set_global_assignment -name PROJECT_CREATION_TIME_DATE \"$datestr\"\n";
    ...
    for $sdcfn($rev->{sdcfiles}){
        printf QSFH "set_global_assignment -name SDC_FILE $sdcfn\n";
    }
